...
Детальные методы заканчиваются на Response: `CreateQueueResponse()`, `PurgeQueueResponse()` и пр. Эти методы возвращают детальные **данные ответа**с: код ответа http, тело запроса, заголовки http и т.п. Их можно детально разобрать, залогировать или написать более подробную обработку ответа самостоятельно:
