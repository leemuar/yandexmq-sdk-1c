...
Функция НайтиСимволСКонца(СтрокаПоиска, ИскомыйСимвол)
	
	ПозицияСимвола = 0;
	
	ТекПозиция = СтрДлина(СтрокаПоиска);
	Пока ТекПозиция > 0 Цикл
		ТекСимвол = Сред(СтрокаПоиска, ТекПозиция, 1);
		Если ТекСимвол = ИскомыйСимвол Тогда
			ПозицияСимвола = ТекПозиция;
			Прервать;
		КонецЕсли;
...
Функция пе_СтрЗаканчиваетсяНа(Строка, СтрокаПоиска) Экспорт
