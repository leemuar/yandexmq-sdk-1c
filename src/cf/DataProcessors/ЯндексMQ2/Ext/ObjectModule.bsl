...
#Область РусскоязычныеМетодыAPI



// Получает список URL очередей
//
// Вызывает метод ListQueues сервиса
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueNamePrefix	-	Строка	-	(опционально) Префикс для фильтрации имен очередей. Чувствителен к регистру.
//
// Возвращаемое значение:
//  Неопределено - если запрос завершился ошибкой
//  Структура:
//        * QueueUrls – Массив из Строка  - массив URL очередей
//        * NextToken – Строка            - токен пагинации, используется при получении списка очередей частями
//
Функция СписокОчередей( Знач ПараметрыЗапроса = Неопределено ) Экспорт
	
	Возврат ListQueues( ПараметрыЗапроса );
...
//  См. ВызватьМетод
//
Функция СписокОчередейОтвет( Знач ПараметрыЗапроса = Неопределено ) Экспорт

	Возврат ListQueuesResponse( ПараметрыЗапроса );
...
//
// Вызывает метод PurgeQueue сервиса
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl	-	Строка	-	(обязательный) URL очереди
// 
// Возвращаемое значение:
//  Булево - Признак успешности удаления всех сообщений очереди. Истина - удалено успешно, Ложь - при удалении произошла ошибка
//
Функция ОчиститьОчередь( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат PurgeQueue( ПараметрыЗапроса ); 
...
//  См. ВызватьМетод
//
Функция ОчиститьОчередьОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат PurgeQueueResponse( ПараметрыЗапроса ); 
...
// Создает очередь
//
// Вызывает метод CreateQueue сервиса
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueName		-	Строка		-	(обязательный) Имя создаваемой очереди
//		* Attributes	-	Структура	-	(опционально) атрибуты (настройки) создаваемой очереди
//			** DelaySeconds						-	Число	-	(опционально) Время в секундах, на которое сообщения будут скрыты после отправки.  Возможные значения от 0 до 900 секунд (15 минут). Значение по умолчанию: 0.
//			** MaximumMessageSize				-	Число	-	(опционально) Максимальный размер сообщения в байтах. Возможные значения: от 1024 байт (1 КБ) до 262144 байт (256 КБ). Значение по умолчанию: 262144 (256 КБ).
//			** MessageRetentionPeriod			-	Число	-	(опционально) Срок хранения сообщений в секундах. Возможные значения: от 60 секунд (1 минуты) до 1209600 секунд (14 дней). Значение по умолчанию: 345600 (4 дня).
//			** ReceiveMessageWaitTimeSeconds	-	Число	-	(опционально) Время ожидания для метода ReceiveMessage в секундах. Возможные значения: от 0 до 20 секунд. Значение по умолчанию: 0.
//			** RedrivePolicy					-	Строка	-	(опционально) Политика перенаправления сообщений в Dead Letter Queue.
//			** VisibilityTimeout				-	Строка	-	(опционально) Таймаут видимости сообщений в очереди в секундах. Возможные значения: от 0 до 43000 секунд. Значение по умолчанию: 30.
//			** FifoQueue						-	Булево	-	(опционально) Флаг, указывающий что создается очередь FIFO.
//			** ContentBasedDeduplication		-	Булево	-	(опционально)  	Флаг, включающий дедупликацию по содержимому сообщения. Подробнее о дедупликации см. https://yandex.cloud/ru/docs/message-queue/concepts/deduplication#content-based-deduplication
//		* Tags			-	Структура	-	(опционально) теги (метки) создаваемой очереди. Подробнее о метках: https://yandex.cloud/ru/docs/message-queue/concepts/tags
// 
// Возвращаемое значение:
//  Строка - URL созданной очереди
//  Неопределено - Если очередь создать не удалось
//
Функция СоздатьОчередь( Знач ПараметрыЗапроса ) Экспорт

	Возврат CreateQueue( ПараметрыЗапроса );
...
//  См. ВызватьМетод
//
Функция СоздатьОчередьОтвет( Знач ПараметрыЗапроса ) Экспорт

	Возврат CreateQueueResponse( ПараметрыЗапроса );
...
// Удаляет очередь
//
// Вызывает метод DeleteQueue сервиса
//
...
//
// Процесс удаления очереди занимает до 60 секунд. В течение этого времени некоторые запросы, 
// например, отправка сообщений в очередь, могут выполняться, но очередь все равно 
// будет удалена вместе со всеми сообщениями.
//
...
//
// Возвращаемое значение:
//  Булево - Признак успешности удаления очереди. Истина - удалено успешно, Ложь - при удалении произошла ошибка
//
Функция УдалитьОчередь( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат DeleteQueue( ПараметрыЗапроса );
...
//  См. ВызватьМетод
//
Функция УдалитьОчередьОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат DeleteQueueResponse( ПараметрыЗапроса );
	
КонецФункции



// Получает URL очереди
//
// Вызывает метод GetQueueUrl сервиса
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueName	-	Строка	-	(обязательный) имя очереди
// 
// Возвращаемое значение:
//  Строка       - URL очереди в случае успешного запроса
//  Неопределено - в случае ошибки запроса (например, очереди с таким именем не существует)
//
Функция ПолучитьUrlОчереди( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат GetQueueUrl( ПараметрыЗапроса );
...
//  См. ВызватьМетод
//
Функция ПолучитьUrlОчередиОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат GetQueueUrlResponse( ПараметрыЗапроса );
	
КонецФункции



// Получает атрибуты (настройки) указанной очереди
//
// Вызывает метод GetQueueAttributes сервиса
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl	-	Строка	-	(обязательный) URL очереди
//		* AttributeNames	- Массив из Строка -	(опц.) Массив имен атрибутов, значения которые требуется получить
// 
// Возвращаемое значение:
//  Неопределено - в случае ошибки запроса
//  Структура - Данные атрибутов очереди. Пары "Имя атрибута" - "значение атрибута" 
//
Функция ПолучитьАтрибутыОчереди( Знач ПараметрыЗапроса ) Экспорт

	Возврат GetQueueAttributes( ПараметрыЗапроса );
...
//  См. ВызватьМетод
//
Функция ПолучитьАтрибутыОчередиОтвет( Знач ПараметрыЗапроса ) Экспорт

	Возврат GetQueueAttributesResponse( ПараметрыЗапроса );

КонецФункции



// Устанавливает атрибуты (настройки) очереди
//
// Вызывает метод SetQueueAttributes сервиса
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl	-	Строка	-	(обязательный) URL очереди
//		* Attributes	-	Структура	-	(обязательный) Атрибуты очереди. Пары "Имя атрибута" - "значение"
// 
// Возвращаемое значение:
//  Булево - Признак успешности установки атрибутов очереди. Истина - если успешно, Ложь - если не удалось
//
Функция УстановитьАтрибутыОчереди( Знач ПараметрыЗапроса ) Экспорт
	
   Возврат SetQueueAttributes( ПараметрыЗапроса ); 
...
//  См. ВызватьМетод
//
Функция УстановитьАтрибутыОчередиОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат SetQueueAttributesResponse( ПараметрыЗапроса );
	
КонецФункции



// Сохраняет сообщение в указанной очереди
//
// Вызывает метод SendMessage сервиса
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl		-	Строка	-	(обязательный) URL очереди
//		* MessageBody	-	Строка	-	(обязательный) Тело сообщения
//		* DelaySeconds	-	Число	-	(опционально) Время в секундах, на которое сообщение будет скрыто после отправки.
//		* MessageAttributes	-	Структура	-	(опционально) Пользовательские атрибуты сообщения
//		* MessageDeduplicationId	-	Строка	-	(только для очередей FIFO) Идентификатор токена для дедупликации сообщений
//		* MessageGroupId	-	Строка	-	(только для очередей FIFO) Идентификатор группы сообщений, используется в очередях FIFO. Максимальная длина — 128 символов. Разрешено использование цифр, больших и маленьких латинских букв и знаков пунктуации. 
// 
// Возвращаемое значение:
//  Структура:
//        * MD5OfMessageAttributes – Строка - MD5-хэш строки атрибута
//        * MD5OfMessageBody       – Строка - MD5-хэш тела сообщения
//        * SequenceNumber         – Строка - Номер сообщения, используется в очередях FIFO.
//        * MessageId              – Строка - Идентификатор отправленного сообщения
//  Неопределено - если отправка сообщения завершилась ошибкой
//
Функция ОтправитьСообщение( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат SendMessage( ПараметрыЗапроса );
...
//  См. ВызватьМетод
//
Функция ОтправитьСообщениеОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат SendMessageResponse( ПараметрыЗапроса );
	
КонецФункции



// Получает одно или несколько сообщений из указанной очереди
//
// Вызывает метод RecieveMessage сервиса
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl	-	Строка	-	(обязательный) URL очереди
//		* MaxNumberOfMessages	-	Число	-	(опционально) Максимальное количество сообщений, которое будет принято
//		* AttributeNames	-	Массив из Строка	-	(опционально) Массив имен атрибутов сообщения, которые требуется вернуть в ответе на запрос
//		* ReceiveRequestAttemptId	-	Строка	-	(опционально) дентификатор для повтора попытки получения сообщений из FIFO-очереди
//		* VisibilityTimeout	-	Число	-	(опционально) Таймаут видимости получаемого сообщения
//		* WaitTimeSeconds	-	Число	-	(обязательный) Время ожидания доставки сообщения в очередь в секундах
// 
// Возвращаемое значение:
//  Неопределено - если не удалось получить сообщения
//  Массив - Массив из Структур с данными полученных сообщений
//
Функция ПолучитьСообщение( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат RecieveMessage( ПараметрыЗапроса );
...
//  См. ВызватьМетод
//
Функция ПолучитьСообщениеОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат RecieveMessageResponse( ПараметрыЗапроса );
...
//
// Сообщает очереди, что полученное сообщение обработано, и его необходимо убрать из очереди 
// Вызывает метод DeleteMessage сервиса 
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl	-	Строка	-	(обязательный) URL очереди, в которой находится сообщение
//		* ReceiptHandle	-	Строка	-	(обязательный) Идентификатор удаляемого сообщения. Параметр ReceiptHandle из запроса ReceiveMessage
// 
// Возвращаемое значение:
//  Булево - Признак успешности удаления. Истина - удалено успешно, Ложь - произошла ошибка при удалении
//
Функция УдалитьСообщение( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат DeleteMessage( ПараметрыЗапроса );
...
//  См. ВызватьМетод
//
Функция УдалитьСообщениеОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат DeleteMessageResponse( ПараметрыЗапроса );	
	
КонецФункции



// Изменяет таймаут видимости указанного сообщения в очереди
//
// Вызывает метод ChangeMessageVisibility сервиса 
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl			-	Строка	-	(обязательный) URL очереди, в которой находится сообщение.
//		* ReceiptHandle		-	Строка	-	(обязательный) Параметр ReceiptHandle из запроса ReceiveMessage.
//		* VisibilityTimeout	-	Число	-	(обязательный) Новое значение таймаута видимости сообщений в очереди в секундах. Возможные значения: от 0 до 43200 секунд.
//
// Возвращаемое значение:
//  Булево - Признак успешности изменения. Истина - если успешно, Ложь - если не удалось изменить
//
Функция ИзменитьВидимостьСообщения( Знач ПараметрыЗапроса ) Экспорт

	Возврат ChangeMessageVisibility( ПараметрыЗапроса );
...
//  См. ВызватьМетод
//
Функция ИзменитьВидимостьСообщенияОтвет( Знач ПараметрыЗапроса ) Экспорт

	Возврат ChangeMessageVisibilityResponse( ПараметрыЗапроса );

КонецФункции



// Метод для задания меток (tags) очереди. 
//
// Изменение меток может занять до 60 секунд.
// Новая метка с ключом, идентичным ключу существующей метки, заменяет существующую метку.
//
// Вызывает метод TagQueue сервиса 
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl	-	Строка		-	(обязательный) URL очереди
//		* Tags		-	Структура	-	(обязательный) Метки очереди. Пары "Имя метки" - "значение метки"
// Возвращаемое значение:
//  См. ВызватьМетод
//
Функция ЗадатьМеткиОчередиОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат TagQueueResponse( ПараметрыЗапроса );
...
//		* Tags		-	Структура	-	(обязательный) Метки очереди. Пары "Имя метки" - "значение метки"
// Возвращаемое значение:
//  Булево - признак успешности установки меток: Истина - успешно, Ложь - произошла ошибка
//
Функция ЗадатьМеткиОчереди( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат TagQueue( ПараметрыЗапроса );
	
КонецФункции



// Метод для удаления меток очереди. 
//
// Вызывает метод UntagQueue сервиса
// Изменение меток может занять до 60 секунд.
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl	-	Строка		-	(обязательный) URL очереди
//		* TagKeys	-	Массив из Строка	-	(обязательный) Имена меток
// Возвращаемое значение:
//  См. ВызватьМетод
//
Функция УдалитьМеткиОчередиОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат UntagQueueResponse( ПараметрыЗапроса ); 
...
//
// Параметры:
//  QueryParams - Структура:
//		* QueueUrl	-	Строка		-	(обязательный) URL очереди
//		* TagKeys	-	Массив из Строка	-	(обязательный) Имена меток
// Возвращаемое значение:
//  См. ВызватьМетод
//
Функция УдалитьМеткиОчереди( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат UntagQueue( ПараметрыЗапроса );
	
КонецФункции



// Метод для получения меток очереди 
//
// Вызывает метод ListQueueTags сервиса
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl	-	Строка	-	(обязательный) URL очереди
...
//  См. ВызватьМетод
//
Функция ПолучитьМеткиОчередиОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат ListQueueTagsResponse( ПараметрыЗапроса );
...
// Возвращаемое значение:
//  Неопределено - если запрос завершился ошибкой
//  Структура - метки очереди. Пары "имя метки" - "значение метки"
//
Функция ПолучитьМеткиОчереди( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат ListQueueTags( ПараметрыЗапроса ); 
	
КонецФункции



// Метод для массового изменения времени видимости нескольких сообщений
//
// Вызывает метод ChangeMessageVisibilityBatch сервиса
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl	-	Строка	-	(обязательный) URL очереди
//		* Entries	-	Массив из Структура	-	(обязательный) массив из описаний необходимых изменений
//
// Возвращаемое значение:
//  См. ВызватьМетод
//
Функция ИзменитьВидимостьНесколькихСообщенийОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат ChangeMessageVisibilityBatchResponse( ПараметрыЗапроса );
...
//
// Возвращаемое значение:
//  Неопределено - в случае ошибки при выполнении запроса
//	Структура - данные об успешности изменения для каждого сообщения
//
Функция ИзменитьВидимостьНесколькихСообщений( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат ChangeMessageVisibilityBatch( ПараметрыЗапроса ); 
	
КонецФункции



// Метод для удаления нескольких сообщений из очереди за один запрос
//
// Вызывает метод DeleteMessageBatch сервиса
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl	-	Строка	-	(обязательный) URL очереди, в которой находятся сообщения
//		* Entries	-	Массив из Структура	-	(обязательный) массив описаний удаляемых сообщений
//
// Возвращаемое значение:
//  См. ВызватьМетод
//
Функция УдалитьНесколькоСообщенийОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат DeleteMessageBatchResponse( ПараметрыЗапроса ) ; 
...
// Возвращаемое значение:
//  Неопределено - если запрос завершился ошибкой
//  Структура - описание успешности удаления сообщений
//
Функция УдалитьНесколькоСообщений( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат DeleteMessageBatch( ПараметрыЗапроса ); 
	
КонецФункции



// Метод для отправки нескольких сообщений в очередь за один запрос
//
// Вызывает метод SendMessageBatch сервиса
//
// Параметры:
//  ПараметрыЗапроса - Структура:
//		* QueueUrl	-	Строка	-	(обязательный) URL очереди
//		* Entries	-	Массив из Структура	-	(обязательный) массив описаний отправляемых сообщений
//
// Возвращаемое значение:
//  См. ВызватьМетод
//
Функция ОтправитьНесколькоСообщенийОтвет( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат SendMessageBatchResponse( ПараметрыЗапроса ); 
...
// Возвращаемое значение:
//  Неопределено - если запрос завершился ошибкой
//  Структура - описание успешности отправки сообщений
//
Функция ОтправитьНесколькоСообщений( Знач ПараметрыЗапроса ) Экспорт
	
	Возврат SendMessageBatch( ПараметрыЗапроса );
...
//        * NextToken – Строка            - токен пагинации, используется при получении списка очередей частями
//
Функция ListQueues( Знач QueryParams = Неопределено ) Экспорт

	Ответ = ListQueuesResponse( QueryParams );
	
	Если 200 <> Ответ.КодСостояния Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат КакJson( Ответ, Новый Структура( "ПрочитатьВСоответствие", Ложь ) ); 
...
//  Булево - Признак успешности удаления очереди. Истина - удалено успешно, Ложь - при удалении произошла ошибка
//
Функция DeleteQueue( Знач QueryParams ) Экспорт
	
	Ответ = DeleteQueueResponse( QueryParams );
	Возврат 200 = Ответ.КодСостояния;
...
//  Булево - Признак успешности удаления всех сообщений очереди. Истина - удалено успешно, Ложь - при удалении произошла ошибка
//
Функция PurgeQueue( Знач QueryParams ) Экспорт
	
	Ответ = PurgeQueueResponse( QueryParams );
	Возврат 200 = Ответ.КодСостояния;
...
//  Неопределено - Если очередь создать не удалось
//
Функция CreateQueue( Знач QueryParams ) Экспорт
	
	Ответ = CreateQueueResponse( QueryParams );
	Если 200 <> Ответ.КодСостояния Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат КакJson( Ответ, Новый Структура( "ПрочитатьВСоответствие", Ложь ) )["QueueUrl"];
...
//  Неопределено - в случае ошибки запроса (например, очереди с таким именем не существует)
//
Функция GetQueueUrl( Знач QueryParams ) Экспорт
	
	Ответ = GetQueueUrlResponse( QueryParams );
	Если 200 <> Ответ.КодСостояния Тогда
		Возврат Неопределено;	
	КонецЕсли;
	
	Возврат КакJson( Ответ )["QueueUrl"];
...
//  Структура - Данные атрибутов очереди. Пары "Имя атрибута" - "значение атрибута" 
//
Функция GetQueueAttributes( Знач QueryParams ) Экспорт
	
	Ответ = GetQueueAttributesResponse( QueryParams );
	
	Если 200 <> Ответ.КодСостояния Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат КакJson( Ответ, Новый Структура( "ПрочитатьВСоответствие", Ложь ) )["Attributes"];
...
//  Булево - Признак успешности установки атрибутов очереди. Истина - если успешно, Ложь - если не удалось
//
Функция SetQueueAttributes( Знач QueryParams ) Экспорт
	
	Ответ = SetQueueAttributesResponse( QueryParams );
	Возврат 200 = Ответ.КодСостояния;
...
//  Неопределено - если отправка сообщения завершилась ошибкой
//
Функция SendMessage( Знач QueryParams ) Экспорт
	
	Ответ = SendMessageResponse( QueryParams );
	Если 200 <> Ответ.КодСостояния Тогда
		Возврат Неопределено;
	КонецЕсли;
...
//  Массив - Массив из Структур с данными полученных сообщений
//
Функция RecieveMessage( Знач QueryParams ) Экспорт
	
	Ответ = RecieveMessageResponse( QueryParams );
	Если 200 <> Ответ.КодСостояния Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ДанныеОтвета = КакJson( Ответ, Новый Структура( "ПрочитатьВСоответствие", Ложь ) );
	
	Если НЕ ДанныеОтвета.Свойство( "Messages" ) Тогда
		Возврат Новый Массив;
	КонецЕсли;
	
	Возврат ДанныеОтвета[ "Messages" ];
...
//  Булево - Признак успешности удаления. Истина - удалено успешно, Ложь - произошла ошибка при удалении
//
Функция DeleteMessage( Знач QueryParams ) Экспорт
	
	Ответ = DeleteMessageResponse( QueryParams );
	Возврат 200 = Ответ.КодСостояния;
...
//  Булево - Признак успешности изменения. Истина - если успешно, Ложь - если не удалось изменить
//
Функция ChangeMessageVisibility( Знач QueryParams ) Экспорт
	
	Ответ = ChangeMessageVisibilityResponse( QueryParams );
	Возврат 200 = Ответ.КодСостояния;
...
//  См. ВызватьМетод
//
Функция ListQueuesResponse( Знач QueryParams = Неопределено ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "ListQueues", QueryParams ); 
...
//  См. ВызватьМетод
//
Функция CreateQueueResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "CreateQueue", QueryParams ); 
...
//  См. ВызватьМетод
//
Функция DeleteQueueResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "DeleteQueue", QueryParams );
...
//  См. ВызватьМетод
//
Функция PurgeQueueResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "PurgeQueue", QueryParams );
...
//  См. ВызватьМетод
//
Функция GetQueueUrlResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "GetQueueUrl", QueryParams );
...
//  См. ВызватьМетод
//
Функция GetQueueAttributesResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "GetQueueAttributes", QueryParams );
...
//  См. ВызватьМетод
//
Функция SetQueueAttributesResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "SetQueueAttributes", QueryParams );
...
//  См. ВызватьМетод
//
Функция SendMessageResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "SendMessage", QueryParams );
...
//  См. ВызватьМетод
//
Функция RecieveMessageResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "ReceiveMessage", QueryParams );
...
//  См. ВызватьМетод
//
Функция DeleteMessageResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "DeleteMessage", QueryParams );
...
//  См. ВызватьМетод
//
Функция ChangeMessageVisibilityResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "ChangeMessageVisibility", QueryParams );
...
//  См. ВызватьМетод
//
Функция TagQueueResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "TagQueue", QueryParams );
...
//  Булево - признак успешности установки меток: Истина - успешно, Ложь - произошла ошибка
//
Функция TagQueue( Знач QueryParams ) Экспорт
	
	Ответ = TagQueueResponse( QueryParams );
...
//  См. ВызватьМетод
//
Функция UntagQueueResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "UntagQueue", QueryParams ); 
...
//  См. ВызватьМетод
//
Функция UntagQueue( Знач QueryParams ) Экспорт
	
	Ответ = UntagQueueResponse( QueryParams );
	Возврат Ответ.КодСостояния = 200;
...
//  См. ВызватьМетод
//
Функция ListQueueTagsResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "ListQueueTags", QueryParams );
...
//  Структура - метки очереди. Пары "имя метки" - "значение метки"
//
Функция ListQueueTags( Знач QueryParams ) Экспорт
	
	Ответ = ListQueueTagsResponse( QueryParams );
	
	Если 200 <> Ответ.КодСостояния Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат КакJson( Ответ, Новый Структура( "ПрочитатьВСоответствие", Ложь ) )["Tags"]; 
...
//  См. ВызватьМетод
//
Функция ChangeMessageVisibilityBatchResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "ChangeMessageVisibilityBatch", QueryParams ); 
...
//	Структура - данные об успешности изменения для каждого сообщения
//
Функция ChangeMessageVisibilityBatch( Знач QueryParams ) Экспорт
	
	Ответ = ВыполнитьЗапросAwsJson( "ChangeMessageVisibilityBatch", QueryParams );
...
//  См. ВызватьМетод
//
Функция DeleteMessageBatchResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "DeleteMessageBatch", QueryParams ); 
...
//  Структура - описание успешности удаления сообщений
//
Функция DeleteMessageBatch( Знач QueryParams ) Экспорт
	
	Ответ = DeleteMessageBatchResponse( QueryParams );
...
//  См. ВызватьМетод
//
Функция SendMessageBatchResponse( Знач QueryParams ) Экспорт
	
	Возврат ВыполнитьЗапросAwsJson( "SendMessageBatch", QueryParams ); 
...
//  Структура - описание успешности отправки сообщений
//
Функция SendMessageBatch( Знач QueryParams ) Экспорт
	
	Ответ = SendMessageBatchResponse( QueryParams );
...
Функция РезультатПоследнегоЗапроса() Экспорт
	
	Возврат _РезультатПоследнегоЗапроса;
	
КонецФункции



Функция КакПараметр( Знач ЗначениеПараметра )

	Возврат Формат( ЗначениеПараметра, "ЧДЦ=0; ЧН=0; ЧГ=; БЛ=false; БИ=true" );
...
Процедура Инициализация()
	...
	// параметры для работы с Yandex Message Queue
	_Endpoint  = "https://message-queue.api.cloud.yandex.net/";
	_Version   = "2012-11-05";
	_Region    = "ru-central1";
...
Функция ПодготовитьПараметрыЗапроса( Знач ВходящиеПараметрыЗапроса )
	
	ПодготовленныеПараметры = Новый Соответствие;

	// имена параметров, которые мы НЕ преобразуем к строке
	Пропускать = Новый Структура;
	Пропускать.Вставить( "VisibilityTimeout" );
	Пропускать.Вставить( "MaxNumberOfMessages" );
	
	Если НЕ ЗначениеЗаполнено( ВходящиеПараметрыЗапроса ) Тогда
		Возврат ПодготовленныеПараметры;
	КонецЕсли;
	
	Для Каждого КлЗнПарам Из ВходящиеПараметрыЗапроса Цикл
		
		Значение = Скопировать( КлЗнПарам.Значение );
		Если ( ТипЗнч(Значение) = Тип("Число") ИЛИ ТипЗнч(Значение) = Тип("Булево") ) И НЕ Пропускать.Свойство( КлЗнПарам.Ключ ) Тогда
			Значение = КакПараметр( Значение ); 
		ИначеЕсли ТипЗнч(Значение) = Тип("Структура") Тогда
			
			Для Каждого КлЗнСтр Из Значение Цикл
				
				Если ( ТипЗнч(КлЗнСтр.Значение) = Тип("Число") Или ТипЗнч(КлЗнСтр.Значение) = Тип("Булево") ) И НЕ Пропускать.Свойство( КлЗнПарам.Ключ ) Тогда
					Значение.Вставить( КлЗнСтр.Ключ, КакПараметр(КлЗнСтр.Значение) );
				КонецЕсли;
		...
		ПодготовленныеПараметры.Вставить( КлЗнПарам.Ключ, Значение );  
...
Функция ВыполнитьЗапросAwsJson( Action, ПараметрыЗапроса )
	
	Данные = ПодготовитьПараметрыЗапроса( ПараметрыЗапроса ); 
	
	Заголовки = Новый Соответствие;
	Заголовки.Вставить( "X-Amz-Target", "AmazonSQS." + Action );
	Заголовки.Вставить( "Content-Type", "application/x-amz-json-1.0" );
	Если ЗначениеЗаполнено( _SecurityToken ) Тогда
		Заголовки.Вставить( "x-amz-security-token", _SecurityToken );
	КонецЕсли;
	
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить( "Аутентификация", НоваяАутентификацияAWS4( _AccessKey, _SecretKey, "sqs", _Region ) );
	ДопПараметры.Вставить( "Заголовки", Заголовки );
	ДопПараметры.Вставить( "Json", Данные ); 
	
	Ответ = Post( _Endpoint, , ДопПараметры );
	
	_РезультатПоследнегоЗапроса = Ответ;
...
//   См. ВызватьМетод
//
Функция Get(URL, ПараметрыЗапроса = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) 
...
//   См. ВызватьМетод
//
Функция Options(URL, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) 
...
//   См. ВызватьМетод
//
Функция Head(URL, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) 
...
//   См. ВызватьМетод
//
Функция Post(URL, Данные = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) 
...
//   См. ВызватьМетод
//
Функция Put(URL, Данные = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) 
...
//   См. ВызватьМетод
//
Функция Patch(URL, Данные = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) 
...
//   См. ВызватьМетод
//
Функция Delete(URL, Данные = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) 
...
//
Функция ВызватьМетод(Метод, URL, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) 
...
//     Параметры преобразования см. ДополнительныеПараметры.ПараметрыПреобразованияJSON
//
Функция PostJson(URL, Json, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) 
...
//     Параметры преобразования см. ДополнительныеПараметры.ПараметрыПреобразованияJSON
//
Функция PutJson(URL, Json, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) 
...
//     Параметры преобразования см. ДополнительныеПараметры.ПараметрыПреобразованияJSON
//
Функция DeleteJson(URL, Json, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) 
...
//
Функция НовыйОтправляемыйФайл(Имя, ИмяФайла, Данные = Неопределено, Тип = Неопределено) 
...
//    * Пароль - Строка - пароль пользователя.
//
Функция НоваяАутентификацияBasic(Пользователь = "", Пароль = "") 
...
//    * Пароль - Строка - пароль пользователя.
//
Функция НоваяАутентификацияDigest(Пользователь = "", Пароль = "") 
...
//    * Регион - Строка - регион, к которому выполняется подключение.
//
Функция НоваяАутентификацияAWS4(КлючДоступа = "", СекретныйКлюч = "", Сервис = "", Регион = "") 
...
//
Функция ОбъектВJson(Объект, Знач ПараметрыПреобразования = Неопределено, Знач ПараметрыЗаписи = Неопределено) 
...
//   Произвольный - значение, десериализованное из JSON.
//
Функция JsonВОбъект(Json, Кодировка = "utf-8", ПараметрыПреобразования = Неопределено) 
...
//
Функция HMAC(Ключ, Данные, Алгоритм) 
...
//
Функция ПредставлениеКодаСостоянияHTTP(КодСостояния) 
...
//
Функция ПреобразованиеJson(Свойство, Значение, ДополнительныеПараметры, Отказ) 
...
//
Функция ВосстановлениеJson(Свойство, Значение, ТипыСвойств) 
...
Функция ДатаИзСтрокиRFC7231(Знач Строка) 
